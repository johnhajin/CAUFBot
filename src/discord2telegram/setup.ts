...
import { loadMappingsFromFile, translateUsername, usernameMapping } from "../settings/usernameTranslate";
const he = require('he');
let dcBot: Client;
 ...
 *
 * @param logger The Logger instance to log messages to
 * @param dcBotInstance The Discord bot
...
export function setup(
	...
	dcBot.on("messageCreate", async message => {
		...
		let senderName = R.compose<any, any>(
			// Make it HTML safe
		...
		// Register username with userId
		const userHolder = dcBot.users.cache.find(user => user.username === senderName)

		if(userHolder){
			const id = userHolder.id
			const mappingsFilePath = path.resolve(__dirname, '../../src/settings/discordIdMappings.txt');
			loadMappingsFromFile(mappingsFilePath)
			
			if (!(senderName in usernameMapping)) {
		...
		// Convert username
		senderName = translateUsername(senderName,"discord")
		...
		if (!R.isEmpty(bridges)) {
			for (const bridge of bridges) {
				...
				if (message.content) {
					...
					// Function to fetch usernames for mentioned user IDs
					async function fetchUsernames(content:string) {
						...
						while ((match = userMentionPattern.exec(content)) !== null) {
							...
							promises.push(dcBot.users.fetch(userId).then(user => {
						...
						users.forEach(user => {
							...
							processedContent = processedContent.replace(new RegExp(mention, 'g'), `@${translateUsername(user.username,"discord")}`);
						});
					...
					const processedMessage = he.decode(md2html(message.content, settings.telegram));


					const finalMessageContent = await fetchUsernames(processedMessage);
