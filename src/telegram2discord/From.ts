...
import { translateUsername } from "../settings/usernameTranslate";
...
export function createFromObjFromUser(user: User) {
	const holder = createFromObj(user.first_name, user.last_name || "", translateUsername(user.username,"telegram") || "");
	return holder
}
